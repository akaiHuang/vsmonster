...
export const WEBVIEW_JS = `(function() {
  ...
  const reasoningLabels = { 'low': '⚡Low', 'medium': '⚖️Med', 'high': '🧠High', 'extra-high': '🔥Max' };
  ...
  function updateActivity(status, lines) {
    ...
    if (activityStatusEl) activityStatusEl.textContent = safeStatus;
    if (activityBodyEl) {
      activityBodyEl.innerHTML = safeLines.map(l => '<div class="activity-line">' + escapeHtml(l) + '</div>').join('');
    }
  ...
  function initCustomSelect(triggerEl, optionsEl, onSelect, maxLen) {
    ...
    optionsEl.addEventListener('click', function(e) { const opt = e.target.closest('.custom-select-option'); if (!opt) return; const val = opt.dataset.value; const reasoning = opt.dataset.reasoning || ''; optionsEl.querySelectorAll('.custom-select-option').forEach(o => o.classList.remove('selected')); opt.classList.add('selected'); const txt = opt.textContent.replace('✓', '').trim().split('⚡')[0].split('⚖')[0].split('🧠')[0].split('🔥')[0].trim(); triggerEl.textContent = maxLen ? truncateText(txt, maxLen) : txt; closeAllSelects(); if (onSelect) onSelect(val, reasoning); });
  }
  initCustomSelect(modeSelectTriggerEl, modeSelectOptionsEl, v => { currentModeValue = v; });
  initCustomSelect(modelSelectTriggerEl, modelSelectOptionsEl, (v, r) => { currentModelValue = v; vscode.postMessage({ type: 'applyModel', value: v, reasoningEffort: r }); }, 12);
  ...
  function showModelPanel(p) { if (!modelSelectOptionsEl || !modelSelectTriggerEl) return; modelSelectOptionsEl.innerHTML = ''; currentModelOptions = p.options || []; const currentReasoning = p.currentReasoning || 'medium'; if (modelHintEl) { modelHintEl.textContent = p.hint || ''; modelHintEl.style.display = p.hint ? 'block' : 'none'; }
    const checkSvg = '<svg viewBox="0 0 20 20"><circle cx="10" cy="10" r="8"/><polyline points="6 10 9 13 14 7"/></svg>';
    function createSimpleOpt(v, l, s, m, isNewGroup) { const wrapper = document.createDocumentFragment(); if (isNewGroup && modelSelectOptionsEl.children.length > 0) { const divider = el('div', 'custom-select-divider'); wrapper.appendChild(divider); } const o = el('div', 'custom-select-option' + (s ? ' selected' : '')); o.dataset.value = v; o.innerHTML = '<span class="check-mark">' + checkSvg + '</span>' + escapeHtml(l) + (m ? '<span class="multiplier-badge' + (m === '0x' ? ' free' : '') + '">' + escapeHtml(m) + '</span>' : ''); wrapper.appendChild(o); return wrapper; }
    function createModelGroup(v, l, m, reasoningOpts, defaultR, currentR, isSelected, isNewGroup) {
      const wrapper = document.createDocumentFragment();
      if (isNewGroup && modelSelectOptionsEl.children.length > 0) { const divider = el('div', 'custom-select-divider'); wrapper.appendChild(divider); }
      const group = el('div', 'model-group' + (isSelected ? ' expanded' : ''));
      const header = el('div', 'model-group-header');
      header.innerHTML = '<span class="expand-icon">▶</span><span class="model-name">' + escapeHtml(l) + '</span>' + (m ? '<span class="multiplier-badge' + (m === '0x' ? ' free' : '') + '">' + escapeHtml(m) + '</span>' : '');
      header.addEventListener('click', function(e) { e.stopPropagation(); const wasExpanded = group.classList.contains('expanded'); document.querySelectorAll('.model-group').forEach(g => g.classList.remove('expanded')); if (!wasExpanded) group.classList.add('expanded'); });
      const opts = el('div', 'model-group-options');
      reasoningOpts.forEach(r => {
        const isSel = isSelected && (r === currentR || (!currentR && r === defaultR));
        const opt = el('div', 'reasoning-option' + (isSel ? ' selected' : ''));
        opt.dataset.value = v;
        opt.dataset.reasoning = r;
        opt.innerHTML = '<span class="check-mark"><svg viewBox="0 0 20 20"><circle cx="10" cy="10" r="8"/><polyline points="6 10 9 13 14 7"/></svg></span>' + (reasoningLabels[r] || r);
        opt.addEventListener('click', function(e) { e.stopPropagation(); currentModelValue = v; modelSelectTriggerEl.textContent = l + (r === 'medium' ? '' : ' ' + (reasoningLabels[r] || r)); document.querySelectorAll('.reasoning-option').forEach(o => o.classList.remove('selected')); document.querySelectorAll('.custom-select-option').forEach(o => o.classList.remove('selected')); opt.classList.add('selected'); closeAllSelects(); vscode.postMessage({ type: 'applyModel', value: v, reasoningEffort: r }); });
        if (isSel) { currentModelValue = v; modelSelectTriggerEl.textContent = l + (r === 'medium' ? '' : ' ' + (reasoningLabels[r] || r)); }
    ...
    if (Array.isArray(p.options) && p.options.length > 0) { p.options.forEach((o, i) => { const v = o.id || o.label || '', l = o.label || o.id || '', m = o.multiplier || '', isNewGroup = o.isNewGroup || false, hasReasoning = o.reasoningOptions && o.reasoningOptions.length > 0; const isSelected = p.current ? (v === p.current) : (i === 0); if (hasReasoning) { modelSelectOptionsEl.appendChild(createModelGroup(v, l, m, o.reasoningOptions, o.defaultReasoning, currentReasoning, isSelected, isNewGroup)); } else { modelSelectOptionsEl.appendChild(createSimpleOpt(v, l, isSelected, m, isNewGroup)); if (isSelected) { modelSelectTriggerEl.textContent = truncateText(l, 12); currentModelValue = v; } } }); } else { modelSelectTriggerEl.textContent = 'No models'; modelSelectOptionsEl.appendChild(createSimpleOpt('', 'No models', true, '', false)); } }
  function clearModelPanel() { if (modelHintEl) modelHintEl.style.display = 'none'; if (reasoningSectionEl) reasoningSectionEl.style.display = 'none'; }
  function applyModel() { if (!currentModelValue) return; const r = reasoningSelectEl ? reasoningSelectEl.value : ''; vscode.postMessage({ type: 'applyModel', value: currentModelValue, reasoningEffort: r }); clearModelPanel(); }
  ...
  function setBusy(b) { isBusy = b; sendEl.disabled = b; sendEl.hidden = b; stopEl.hidden = !b; }
  ...
  function updateQueueStatus(status) {
    ...
    const { running, total, maxConcurrent, usedBudget, budget, budgetExceeded } = status;
    // 工廠模式：total 是總任務數，running 是真正並行執行中的 workers
    if (total === 0 && !budgetExceeded) {
    ...
    let html = '';
    if (budgetExceeded) {
      html = '<span class="queue-badge budget-exceeded">⚠️ 預算已用盡</span>';
    } else if (total > 0) {
      // 🏭 工廠模式：顯示活躍 workers 數量
      html = '<span class="queue-badge running">🏭 ' + running + ' 並行</span>';
      if (total > running) {
        // 顯示總任務數（包含等待的）
        html += '<span class="queue-badge waiting">📋 ' + total + ' 任務</span>';
      }
    }
    if (budget > 0) {
      const pct = Math.min(100, (usedBudget / budget) * 100).toFixed(0);
      html += '<span class="queue-badge budget">💰 ' + usedBudget.toFixed(1) + '/' + budget + ' (' + pct + '%)</span>';
    }
  ...
  window.addEventListener('message', e => {
    const m = e.data;
    if (!m) return;
    if (m.type === 'history') setHistory(m.messages || []);
    else if (m.type === 'append') appendMessage(m.message);
    else if (m.type === 'busy') setBusy(Boolean(m.value));
    else if (m.type === 'activity') updateActivity(String(m.status || 'Idle'), m.lines || []);
    else if (m.type === 'model') setModelLabel(String(m.label || ''));
    else if (m.type === 'modeUpdate') updateModeSelector(String(m.mode || 'agent'));
    else if (m.type === 'confirm') showConfirm(m);
    else if (m.type === 'confirmClear') clearConfirm();
