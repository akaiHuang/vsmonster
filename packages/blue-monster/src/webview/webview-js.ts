...
export const WEBVIEW_JS = `(function() {
  ...
  function showConfirm(p) { if (!confirmPanelEl) return; pendingConfirmId = p.id || ''; pendingConfirmCategory = p.category || ''; const ct = document.getElementById('confirmTitle'), cdt = document.getElementById('confirmDangerType'), ctn = document.getElementById('confirmTaskName'); if (ct) ct.textContent = p.dangerType ? '⚠️ 敏感命令確認' : '⚠️ 執行命令確認'; if (cdt) { cdt.textContent = p.dangerType || ''; cdt.style.display = p.dangerType ? 'inline' : 'none'; } if (ctn) ctn.textContent = p.dangerType || '執行此命令'; if (confirmCommandEl) confirmCommandEl.textContent = p.command || ''; if (confirmCwdEl) { if (p.cwd) { confirmCwdEl.textContent = '📁 ' + p.cwd; confirmCwdEl.style.display = 'block'; } else confirmCwdEl.style.display = 'none'; } if (confirmInputEl) { confirmInputEl.value = ''; confirmInputEl.placeholder = '輸入數字 (1-4) 或直接輸入想法...'; } confirmPanelEl.hidden = false; setTimeout(() => { confirmPanelEl.scrollIntoView({ behavior: 'smooth', block: 'end' }); if (confirmInputEl) confirmInputEl.focus(); }, 100); }
  ...
  function showModelPanel(p) { if (!modelSelectOptionsEl || !modelSelectTriggerEl) return; pendingModelBackend = p.backend || ''; modelSelectOptionsEl.innerHTML = ''; function createOpt(v, l, s) { const o = el('div', 'custom-select-option' + (s ? ' selected' : '')); o.dataset.value = v; o.innerHTML = '<span class="check-mark">✓</span>' + escapeHtml(l); return o; } if (pendingModelBackend === 'cli') { const l = p.current || 'CLI model'; modelSelectTriggerEl.textContent = truncateText(l, 12); modelSelectOptionsEl.appendChild(createOpt(p.current || '', l, true)); currentModelValue = p.current || ''; return; } if (Array.isArray(p.options) && p.options.length > 0) { p.options.forEach((o, i) => { const v = o.id || o.label || '', l = o.label || o.id || '', sel = p.current ? (v === p.current) : (i === 0); modelSelectOptionsEl.appendChild(createOpt(v, l, sel)); if (sel) { modelSelectTriggerEl.textContent = truncateText(l, 12); currentModelValue = v; } }); } else { modelSelectTriggerEl.textContent = 'No models'; modelSelectOptionsEl.appendChild(createOpt('', 'No models', true)); } }
  ...
  function hideBackButton() { if (headerBackEl) headerBackEl.hidden = true; if (headerTitleTextEl) headerTitleTextEl.textContent = 'BlueMonster'; currentHistoryTitle = ''; isViewingHistory = false; }
  function goBackToHistory() { vscode.postMessage({ type: 'newChat' }); hideBackButton(); toggleHistory(); }
  function renderHistory(hs) { if (!hs || hs.length === 0) { historyListEl.innerHTML = (historySearchEl && historySearchEl.value.trim().length > 0) ? '<div class="history-empty">找不到符合的任務</div>' : '<div class="history-empty">尚無任務紀錄</div>'; return; } const tc = hs.length; historyListEl.innerHTML = hs.map((h, i) => { const tid = h.taskId || '#' + String(tc - i).padStart(4, '0'); return '<div class="history-item" data-id="' + escapeHtml(h.id) + '" data-title="' + escapeHtml(h.title) + '" data-taskid="' + escapeHtml(tid) + '"><img class="history-item-icon" src="' + assistantAvatarUrl + '" alt="" /><div class="history-item-content"><div class="history-item-header"><span class="history-item-taskid">' + escapeHtml(tid) + '</span><span class="history-item-title">' + escapeHtml(h.title) + '</span></div><div class="history-item-time">' + escapeHtml(h.date || '') + '</div></div></div>'; }).join(''); historyListEl.querySelectorAll('.history-item').forEach(it => { it.addEventListener('click', function() { const id = this.getAttribute('data-id'); const tt = this.getAttribute('data-title'); if (id) { vscode.postMessage({ type: 'loadHistory', id: id }); historyPanelEl.hidden = true; showBackButton(tt || 'Task'); } }); }); }
  ...
  window.addEventListener('message', e => { const m = e.data; if (!m) return; if (m.type === 'history') setHistory(m.messages || []); else if (m.type === 'append') appendMessage(m.message); else if (m.type === 'busy') setBusy(Boolean(m.value)); else if (m.type === 'thinking') { if (m.reset) { resetThinking(String(m.text || '')); setThinkingVisible(true); } else if (m.text) { appendThinking(String(m.text || '')); setThinkingVisible(true); } if (m.done) setThinkingVisible(false); } else if (m.type === 'model') setModelLabel(String(m.label || '')); else if (m.type === 'confirm') showConfirm(m); else if (m.type === 'confirmClear') clearConfirm(); else if (m.type === 'choice') showChoicePanel(m); else if (m.type === 'choiceClear') clearChoicePanel(); else if (m.type === 'modelOptions') showModelPanel(m); else if (m.type === 'chatHistories') renderHistory(m.histories || []); else if (m.type === 'toast') showToast(m.text); else if (m.type === 'filesSelected') { pendingFiles = pendingFiles.concat(m.files || []); updateImagePreview(); } });
