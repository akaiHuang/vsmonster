import { Client, GatewayIntentBits, Events, Message, Interaction, REST, Routes, SlashCommandBuilder } from 'discord.js';
...
export class DiscordChannel implements ChannelAdapter {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      logger.info(`Discord Bot initialized: ${this.client.user?.tag}`);
    } catch (error) {
  ...
  async sendMessage(userId: string, message: OutgoingMessage): Promise<void> {
    try {
      ...
      logger.error(`Failed to send message to Discord user ${userId}:`, error);
      throw error;
    }
  }
  ...
  async sendTextMessage(userId: string, text: string, channelId?: string): Promise<void> {
    try {
      ...
      logger.error(`Failed to send text to Discord:`, error);
      throw error;
    }
  }
  ...
  async replyToInteraction(interactionId: string, text: string): Promise<void> {
    // 這需要在 interaction 仍有效時使用
    // 通常透過 webhook 回覆
    logger.warn('replyToInteraction not implemented for cached interactions');
  }
